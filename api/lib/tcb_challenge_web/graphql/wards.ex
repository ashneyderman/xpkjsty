defmodule TcbChallengeWeb.Graphql.Wards do
  ...
  def fetch_ward(_parent, args, _resolution) do
    ...
    result = case {ward_id, alderman_name} do
      {nil, nil} ->
        {:error, "Neither ward ID nor alderman's name provided as fetch criteria. Provide at least one to fetch ward information."}
      {ward_id, nil}       -> Socrata.fetch_wards_by_ids(ward_id)
      {nil, alderman_name} -> Socrata.fetch_wards_by_alderman(alderman_name)
      {_, _} -> 
        {:error, "Only ward ID or alderman's name allowed as fetch criteria. Provide at most one to fetch ward information."}
    end
