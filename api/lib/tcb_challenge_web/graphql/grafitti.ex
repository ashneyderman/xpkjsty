defmodule TcbChallengeWeb.Graphql.Grafitti do
  ...
  defp derive_wards_argument(args) do
    ...
    result =
      case {ward_ids, alderman_names} do
        {nil, nil} ->
          {:error,
           "Either a list of ward IDs or a list of alderman names has to be specified. Neither was specfied."}

        {ward_ids, nil} ->
          Socrata.fetch_wards_by_ids(ward_ids)

        {nil, alderman_names} ->
          derive_wards_from_aldermans(alderman_names)

        {_, _} ->
          {:error,
           "Either a list of ward IDs or a list of alderman names has to be specified. Both at the same can not be specified."}
      end
