defmodule SODA.QueryBuilder do
  ...
  def date_extract_y(field, display) when is_binary(field) and is_binary(display),
    do: {:date_extract_y, field, :as, display}

  def date_extract_m(field) when is_binary(field), do: {:date_extract_m, field}

  def date_extract_m(field, display) when is_binary(field) and is_binary(display),
    do: {:date_extract_m, field, :as, display}

  def count(), do: {:count, "*"}

  def count(field, display) when is_binary(field) and is_binary(display),
