defmodule SODA.Query do
  ...
  defp select_field({:date_extract_y, field}) when is_binary(field),
    do: "date_extract_y(#{field})"

  defp select_field({:date_extract_y, field, :as, display})
       when is_binary(field) and is_binary(display),
       do: "date_extract_y(#{field}) AS #{display}"

  defp select_field({:date_extract_m, field}) when is_binary(field),
    do: "date_extract_m(#{field})"

  defp select_field({:date_extract_m, field, :as, display})
       when is_binary(field) and is_binary(display),
       do: "date_extract_m(#{field}) AS #{display}"

  defp select_field({:count, field}) when is_binary(field), do: "count(#{field})"

  defp select_field({:count, field, :as, display}) when is_binary(field) and is_binary(display),
    do: "count(#{field}) AS #{display}"

  defp select_field(field) when is_binary(field), do: field

  defp select_field({:as, field, display}) when is_binary(field) and is_binary(display),
    do: "#{field} AS #{display}"
  ...
  defp where_cond({:between, field, v0, v1}) when is_binary(field),
    do: "#{field} BETWEEN #{v0} AND #{v1}"
