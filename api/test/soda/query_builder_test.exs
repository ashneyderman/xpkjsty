defmodule TcbChallenge.SODA.QueryBuilderTest do
  ...
  test "select work as expected" do
    ...
    assert select(date_extract_y("field1", "myfield")).select == [{:date_extract_y, "field1", :as, "myfield"}]
    assert select(date_extract_m("field1")).select == [{:date_extract_m, "field1"}]
    assert select(date_extract_m("field1", "myfield")).select == [{:date_extract_m, "field1", :as, "myfield"}]
  end
